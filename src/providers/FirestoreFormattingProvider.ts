...
export class FirestoreFormattingProvider
  ...
  async provideDocumentFormattingEdits(
    ...
    if (
      workspace
        .getConfiguration("firestorerules")
        .get("usePrettierFormatter") ||
      false
    ) {
    ...
    try {
      ...
      tokenizedDoc.forEach((token, line) => {
        let numberOfIndentations = Math.max(
          ...token.map((item) =>
            item.scopes.reduce(
              (count, scope) =>
                indentationScopes.includes(scope) ? count + 1 : count,
              0,
            ),
          ),
        );
        ...
        const currentNumberOfIndentationWhitespace =
          document.lineAt(line).text.length -
          document.lineAt(line).text.trimLeft().length;
        ...
        results.push(
          TextEdit.replace(
            new Range(
              new Position(line, 0),
              new Position(line, currentNumberOfIndentationWhitespace),
            ),
